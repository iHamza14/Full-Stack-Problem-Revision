...
exports.getRevisionSolves = void 0;
/**
...
function getISTDayRangeUTCFromISTDate(istDate) {
...
const getRevisionSolves = async (userId) => {
    ...
    const { startUTC: yStart, endUTC: yEnd } = getISTDayRangeUTCFromISTDate(yesterdayIST);
    const { startUTC: wStart, endUTC: wEnd } = getISTDayRangeUTCFromISTDate(weekBackIST);
    const { startUTC: mStart, endUTC: mEnd } = getISTDayRangeUTCFromISTDate(monthBackIST);
    ...
    const [previousDay, previousWeek, previousMonth] = await Promise.all([
        prismac_1.prisma.solve.findMany({
...
exports.getRevisionSolves = getRevisionSolves;
